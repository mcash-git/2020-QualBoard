...
import { buttonTypes } from 'shared/components/split-button-model';
import { ButtonModel } from 'shared/components/button-model';
  ...
  constructor(event = {}) {
    ...
    super({
      ...
      detailItems: [{
        ...
        expandContent: {
          viewModel: 'observer/project/events/event-card-expand-content-schedule',
        ...
        expandContent: {
          viewModel: 'observer/project/events/event-card-expand-content-post-stats',
      ...
      button: buildButtonModel(event),
    });
    this.event = event;
  }
}

function buildButtonModel(event) {
  const baseUrl = `/#/observer/projects/${event.projectId}/${typeToUrlFragment(event.type)}/${event.id}`;
